pipeline {
  agent {
    kubernetes {
      label 'bblfsh-performance'
      defaultContainer 'bblfsh-performance'
      yaml """
spec:
  nodeSelector:
...
"""
  ...
  stages {
    ...
    stage('Run end-to-end benchmark') {
      when { branch 'master' }
      steps {
        sh "/root/bblfsh-performance end-to-end --language=${env.DRIVER_LANGUAGE} --commit=${env.GIT_COMMIT} --extension=${env.DRIVER_LANGUAGE_EXTENSION} --docker-tag=latest --custom-driver=true --storage=prom ${env.DRIVER_SRC_FIXTURES}"
      }
    }
  }
}
